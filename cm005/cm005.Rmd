...
```{r, echo=FALSE}
#NOTES:
  # - plot() = lots of coding by hand 
  # - lattice aimed at creating multiple panels of plots
  # - ggplot2 = very easy + powerful (based on theory of grammar of graphics)
  # - plotly = interactive plotting

  #aesthetic mappings: size, positioning, colour, shape <linking variables w these>
  #geometric objects: what is being drawn on the plot
  #scales: linear, logartihmic
  #stat transform: linear regression, box plot
  #facet = breaking up data into subpanels

  #ggplot(gapminder, aes(xname, yname)) <-required aesthetics
  #geompoint(alpha = size) <- geometric object = points
  #scale_x_log10("x label name", labels = scales::dollar_format())<- scale x log +                                                                         dollar signs
  #filter(year == 2007) <- choose spec column ~ gapminder$year == 2007
...
```{r, echo=FALSE}
#NOTES:
# - qplot() // quickplot() useful for very quick vector plotting
# - ggplot(data, aes(x, y)
# - aes(x, y, colour, size, shape)
# - geom_name (point, bar, line) <-- alpha = 0.1 means 10 points to make opaque
# TO SCALE: scale_aes_scName(axisLabel, scales::{format:units/$/etc})
```
...
p <- ggplot(mauna, aes(y = month)) +
  geom_line(aes(x = month, y = conc)) +
  ylab('Concentration') +
  geom_point(colour = "green", alpha = 0.1)
```
